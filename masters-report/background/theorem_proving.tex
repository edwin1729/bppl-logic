...
Examples in this section are based on the excellent introduction to the calculus of inductive constructions by \cite{coc}.

Foundational theorem proving is proving theorems with only the axioms (foundations) of mathematics assumed. Likewise foundational software verification
for proving theorems (properties) of programs.
...
The type of identity functions A is given by $\Pi (\_ : A), A$. We use the notation $A \to A$ in this case since it is not dependent. For completeness the identity function is typed
...
its definition therefore dependent. Again for completeness we type the term which is the polymorphic identity function: 
...
to the type $A \to B$. This is justified by the meaning of implication to be being able to prove $B$ given a proof of $A$. Forall quantification (also) corresponds to dependent product types:
given some $T : \mathbf{Type_i}$ and $P : \mathbf{Prop}$, the logical statement $\forall x : T, P(x)$ corresponds to the type $\Pi (x : T), P(x)$. This also make sense since to prove a
...
above example. With that intuition in mind lets show how some other logical connectives are represented:

As seen in this example, the standard recipe in constructive mathematics is to specify a proposition by what is required prove it. In the next example we
see representation also arises from what the connective enables us to prove. Take $\mathrm{False} := \forall (P : Prop). P : Prop$.
First notice that if we have a proof of $\mathrm{False}$, that is a term of type $\forall (P : Prop). P$, we would get the proof of every any proposition $P$,
out of that, encoding false entails anything. Also, it is not possible to construct a term of type $\forall (P : Prop). P$, ie, there is no proof of $\mathrm{False}$.
A strange thing that one might notice is that we are quantifying over all everything in $Prop$, in order to obtain something of type $Prop$.
...
\inferrule* [right=\textsc{Type-Prop}]
  {\Gamma \vdash}
  {\Gamma \vdash \mathrm{Prop} : \mathrm{Type}_1}
&
\inferrule* [right=\textsc{Type-Form}]
  {\Gamma \vdash}
  {\Gamma \vdash \mathrm{Type}_i : \mathrm{Type}_{i+1}}
\\[1.2em]
...
\inferrule* [right=\textsc{Pi-Prop}]
  {\Gamma, x : A \vdash B : \mathrm{Prop}}
  {\Gamma \vdash \Pi x : A, B : \mathrm{Prop}}
\\[1.2em]

\inferrule* [right=\textsc{Pi-Type}]
  {\Gamma \vdash A : \mathrm{Type}_i \\ \Gamma, x : A \vdash B : \mathrm{Type}_j}
  {\Gamma \vdash \Pi x : A, B : \mathrm{Type}_{\max(i,j)}}
&
...
\caption{Inference rules for the purely functional part of CIC \citep{coc}}
\end{figure}

We notice that there are two typing rules for the dependent product: \texttt{PI-PROP} for sort Prop and \texttt{PI-TYPE} for all other sorts. \texttt{PI-TYPE}
...
\texttt{PI-PROP}, which says that \texttt{PI-PROP} says that regardless of the domain, if the codomain is in Prop, then the $\Pi$-type stays in Prop.
We say that the sort Prop is impredicative, because elements in it are defined by quantifying over Prop itself! Remembering 
$\mathrm{False} := \Pi P : Prop, P : Prop$, we see exactly this. And suppose that we instead were to type this with the rule \texttt{PI-TYPE} and 
$Prop \triangleq Type_0$, then the
domain, $Prop : Type_1$ and codomain $P : Prop$ would give the $\Pi$-type sort $Type_1$, making Prop \emph{predicative}. However, we would like $False : Prop$,
so the impredicativity of Prop is desirable. Let's also recall the definition of forall quantification which also depended on predicativity: $\forall x : T, P$,
we can quanitfy over values in much larger universes or quantify over all Props and still end up with a type in Prop. Impredicativity is powerful but fragile.
It can be shown that if $Type_1$ is also impredicative, then this type theory would become inconsistent \citep{coquand}.

As a final example lets have a look at $A \wedge B \triangleq ∀(P : Prop). (A → B → P) → P : Prop$. Or making things a little more explicit:
$\wedge := λ(A B : Prop). ∀(P : Prop). (A → B → P) → P : Prop \to Prop \to Prop$, where we are first using lambda abstractions and then a $\Pi$-type construction.

\subsubsection{Proof Relevance}
Finally, Proof Relevance separates the type theories of Lean and Rocq. Lean is Proof irrelevant meaning terms of types of Prop, are definitionally equal. So
...
the equivalence of two \lean{MeasurableFun}s is determined solely by the equality of the \lean{toFun}, since \lean{meas} is a (dependent) proof and will be 
definitionally equal if \lean{toFun} is equal. In practice this is incredibly convenient, and is applied automatically throughout the codebase to simplify proof
...
The choices made in lean's type theory to bring about proof irrelevance does have some major theoretical drawbacks such as leading to undecidability of definitional
equality \citep[see chatper 3]{mario}, which however don't seem to matter too much in practice.
...
predicativity and proof relevance, both of which I encountered over the course of the project without have previously heard of it. Interestingly they also cleanly separate 3 
of the most significant proof assistants. They will be mentioned breifly where relevant but is not particularly important in understanding most of this report.
...
higher level, and the use of metaprogramming in Proof assistants, is motivated by the desire to remove the mental overhead of mechanised proofs, move closer
to the style of pen and paper proofs.

``Tactics'' are metaprograms meant for constructing proof terms. A Tactic block is started in Lean using $by$ syntax: in our code if we wanted to
...
\lean{TacticM}. Inside a tactic block a sequence of \lean{TacticM} monads are composed, each monad constructed using one Lean's ecosystem of tactics,
...
types lambda calculus, on top of lambda abstraction and application. Terms are called \lean{Expr} in lean. We introduce the constructor \lean{mvar : MVarID \to Expr}.
For building an expression given a metavariable. This is at the heart of how tactics work. Tactics communicate to eachother of what has been proven and
...
we are looking to construct). As soon as the tactic block starts with \lean{by}, a metavariable \texttt{?m1} which is unassigned but has type \lean{f (f a) = a}.
Now we look at the state after the first \lean{apply}. The types of \lean{Eq.trans} and \texttt{?m1} can be unified, so the tactic succeeds and we have new proof
state as follows:
...
?b  := f a
\end{leancode}
Only ?m3 needs to be assigned which is taken care of the final \texttt{apply}. At the end of the tactic block, we obtain the term
