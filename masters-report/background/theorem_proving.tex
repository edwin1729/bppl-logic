...
\texttt{TacticM}. Inside a tactic block a sequence of \texttt{TacticM} monads are composed, each monad constructed using one Lean's ecosystem of tactics,
...
types lambda calculus, on top of lambda abstraction and application. Terms are called \texttt{Expr} in lean. We introduce the constructor \lean{mvar : MVarID → Expr}.
...
we are looking to construct). As soon as the tactic block starts with \texttt{by}, a metavariable \texttt{?m1} which is unassigned but has type \lean{f (f a) = a}.
