...
The first separation logic was proposed by \cite{og_sl} for reasoning about programs which manipulate a heap. Before we move to probabilistic separation logic which is the main topic
...
are defined with respect to a \emph{resource}. In the original separation logic we have a simple resource which is a finite partial function from some 
...
In this section we see increasingly abstract presentations of separation logic to spot patterns amongst them, and give definitions of characterstics that help classify separation logics.
...
entailments of the form $\vdash P$ or $P \vdash Q$. The \emph{logic of bunched implications} (BI) as presented by \cite{bi} was what classical separation logic was
...
The rest of this section reproduces some figures from \citep{mosel}.
...
&
\begin{array}{c}
\text{\textsc{*-Mono}} \\[0.4em]
\frac{P_1 \vdash Q_1 \qquad P_2 \vdash Q_2}{P_1 * P_2 \vdash Q_1 * Q_2}
\end{array}
&
\begin{array}{c}
\text{\textsc{*-Intro}} \\[0.4em]
\frac{P * Q \vdash R}{P \vdash Q \mathbin{-\!*} R}
\end{array}
&
\begin{array}{c}
\text{\textsc{*-Elim}} \\[0.4em]
\frac{P \vdash Q \mathbin{-\!*} R}{P * Q \vdash R}
\end{array}
\end{array}
\]

\Todo{If I add the one\_le, does the KRM become an affine logic generator?}
% Going back to the KRMs, we can now define the standard recipe for constructing an affine separation logic out of a KRM. 

Next we define the affine propositions: $P$ as any proposition satisfying for all Q $P * Q \vdash Q$. In other words $P$ can always be dropped. An affine 
...
$P \vdash P * P$. \Todo{Why is this not the definition?}.
...
Because of this Iris treats intuitionistic propositions specially, keep them separate from the rest of the ``spatial'' propositions in the premise. So a user
...
In short it is engineered to provide an interface which mimics the natural reasoning style of user. The IPM understands separation logic and endeavours to automate common
boring tasks.
...
%move to main development.
\subsection{Program logic and Weakest precondition}
