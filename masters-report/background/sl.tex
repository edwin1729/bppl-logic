...
programs and assertions we make about the program. We do not go into the specifics of introducing the programming language for this particular logic, since that takes us too far afield.
...
It was hinted in the previous section that $\mathrm{emp}$ and $*$ are like a monoid. In general, what we mean by a ``resource'' can be modelled with some algebraic structure. The spatial connectives $*$ and $\wand$
...
is just a monoid accounting for the fact that $\star$ is a partial operation. So we guard all the laws with existence checks. This turns out to be quite messy in a formalism, as seen for example in the \texttt{assoc}
...
Another class of propositions is persistent propositions: those propositions which may be duplicated to establish the goal. So for instance if $P$ is persistent then
$P \vdash P * P$.
...
\label{MoSeL}
Iris is a general framework for building tactic-based proofs in a separation logic, and MoSeL~\cite{mosel} is the front-end of the Iris Proof Mode through which is
exposed to the user of the logic.
Iris is best understood through the layers of abstraction it provides, summarised in Fig.~\ref{fig:iris-layers}.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
      ...
      lab/.style={font=\scriptsize\itshape, fill=white, inner sep=2pt},
    ]
    \node[box] (krm) {KRM\\\scriptsize(resource model)};
    \node[box, right=of krm] (bi) {BI\\\scriptsize(standard SL connectives)};
    \node[box, right=of bi] (ipm) {Iris Proof Mode};

    \node[box, below=of krm] (prob) {probability-specific\\connectives};
    \node[box, below=of bi]  (tc)   {typeclass\\instances};

    \draw[->] (krm) -- (bi) node[lab, midway, above] {construct};
    \draw[->] (bi)  -- (ipm);
    \draw[->] (krm) -- (prob);
    \draw[->] (prob) -- (tc);
    \draw[->] (tc.east) -- (ipm.south west);
  \end{tikzpicture}
  \caption{The layers of abstraction an instantiator of Iris works through. A resource model
  (a KRM) yields a BI by a predetermined construction, giving the proof mode the connectives and
  properties common to all separation logics. The connectives specific to the object logic are then
  defined and their characteristics registered with Iris as typeclass instances.}
  \label{fig:iris-layers}
\end{figure}

The intended workflow for someone instantiating Iris proceeds along these layers. One starts with a KRM (or a similar algebraic structure) serving as the resource
model, from which there is a predetermined construction of a BI, yielding the standard separation logic connectives: those connectives common to all separation logics. Given this, the Iris proof mode is
aware of the standard ``structural'' properties of a separation logic. For example commutativity and associativity of the separating conjunction, and
its distributivity over $\wedge$. This proof rules are implicitly applied where necessary to simplify goals and the user need not be concerned with
these trivialities. Additional to the generic core, the
instantiator must define the connectives specific to their object logic (in our case the probabilistic connectives such as $\sim$, $\mathrm{own}$ and $\mathbb{E}[\cdot]=e$)
and register their characteristics: whether they are affine, persistent or intuitionistic in the sense of Sec.~\ref{sec:bi}. This is done by providing the corresponding
typeclass instances. Once this is done, the proof mode handles the new connectives correctly: persistent and intuitionistic hypotheses are kept in a separate
intuitionistic context and may be reused freely, while the remaining spatial hypotheses are tracked linearly.

This additional spatial context, sitting along with the usual intuitionistic one, is exactly what distinguishes the Iris proof mode from the base tactic language
of the host prover (Lean or Rocq). The user is presented with an interface that mimics the natural reasoning style of a pen-and-paper separation-logic proof:
introducing variables and hypotheses with a single command, splitting the spatial context across a separating conjunction, applying entailments, and so on, while
Iris automates the bookkeeping these steps would otherwise require. Though such conveniences appear basic, providing them in a manner that is extensible to an arbitrary
separation logic is a difficult problem, and is the central engineering achievement of Iris.

Concrete demonstrations of the proof mode in action introducing variables and spatial hypotheses, and manipulating the spatial context, will be seen in
Chapter~\ref{chap:examples}. We use our embedding of Lilac to interactively verify specifications of probabilistic programs. 
