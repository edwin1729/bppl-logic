...
\textbf{Definition B.5 \cite[Appendix B]{lilac}} A sub-$\sigma$-algebra $\mathcal{F}$ of $\Sigma_\Omega$ has finite footprint if there is some n such that every $F \in \mathcal{F}$
...
\[
\begin{aligned}
(\mathcal{F},\mu) \vDash \mathrm{wp}(M,X\!:\!A.\,Q) &\iff \text{for all } \mathcal{P}_{\mathrm{frame}} \text{ and } \mu \text{ with } \mathcal{P}_{\mathrm{frame}}\bullet\mathcal{P} \sqsubseteq (\Sigma_{I^\mathbb{N}},\mu) \text{ and all } D_{\mathrm{ext}} : \mathrm{RV}\,\llbracket \Delta_{\mathrm{ext}} \rrbracket, \\
&\quad \text{there exists } X : \mathrm{RV}\,A \text{ and } \mathcal{P}' \text{ and } \mu' \text{ with } \mathcal{P}_{\mathrm{frame}}\bullet\mathcal{P}' \sqsubseteq (\Sigma_{I^\mathbb{N}},\mu') \text{ such that } \\
...
\left( \begin{array}{l} \omega \leftarrow \mu;\\ \mathrm{ret}\bigl(D_{\mathrm{ext}}(\omega)\bigr) \end{array} \right) = \left( \begin{array}{l} \omega \leftarrow \mu';\\ \mathrm{ret}\bigl(D_{\mathrm{ext}}(\omega)\bigr) \end{array} \right)
&
\left( \begin{array}{l} \omega \leftarrow \mu;\\ M(\omega) \end{array} \right) = \left( \begin{array}{l} \omega \leftarrow \mu';\\ \mathrm{ret}\bigl(X(\omega)\bigr) \end{array} \right)
\end{array}
\]
...
\inferrule*[right=\textsc{wp-consequence}]
  {P \vdash Q}
  {\mathrm{wp}(M,X,P) \vdash wp(M,X,Q)}

\inferrule*[right=\textsc{wp-frame}]
  {X \notin F}
  {F * \mathrm{wp}(M,X,Q) \vdash wp(M,X,F * Q)}

\inferrule*[right=\textsc{wp-disjunction}]
  {}
  {\mathrm{wp}(M,X,P) \lor wp(M,X,Q) \vdash wp(M,X,P \lor Q)}
...
\inferrule*[right=\textsc{wp-bind}]
  {}
  {\mathrm{wp}([\![M]\!],X,wp([\![N]\!],Y,Q)) \vdash wp([\![X \leftarrow M;\,N]\!],Y,Q)}

\inferrule*[right=\textsc{wp-uniform}]
  {}
  {(\forall_{rv}\,X\!:\!A.\;X\sim\mathrm{Unif}[0,1]\multimap Q)\vdash \mathrm{wp}([\![\mathrm{unif}\,[0,1]]\!],X\!:\!A,Q)}

\inferrule*[right=\textsc{wp-flip}]
  {}
  {(\forall_{rv}\,X\!:\!A.\;X\sim\mathrm{Ber}\,p\multimap Q)\vdash \mathrm{wp}([\![\mathrm{flip}\;p]\!],X\!:\!A,Q)}

\inferrule*[right=\textsc{wp-if}]
  {}
  {\mathrm{wp}([\![M]\!],X\!:\!A.\,wp([\![N]\!],Y\!:\!A.\,Q(\mathrm{if}\;E\;\mathrm{then}\;X\;\mathrm{else}\;Y))) \vdash wp([\![\mathrm{if}\;E\;M\;N]\!],X\!:\!A.\,Q)}
...
Note that in this presentation we have been verbose with stating the isomorphisms going back and forth by splitting and unsplitting the Hilbert cube. In the
informal proof this isomorphism is not mentioned, and it is natural fluidly move in between the two representations, not even noticing that there are must in 
fact be two different representations in a type theory (since the types $I^\mathbb{N}$ and $I^n \times I^\mathbb{N}$ are not equal). Similarly, measures over
one representation fluidly move to the another. An aspect of the latter which was seemingly insignificant and completely intuitive, ended up turning into
an entire file \repo{ProofRules/MeasureProduct} of proofs.

based resources is that it allows   
The properties of FFHC we have explored so far don't really warrant the use of an FFHC. All we really used was the fact that 

\subsection{tour of the wp connective}

Using this let's lay down the core intuition in the proof of \texttt{wp\_meas}. Taking some $\mathcal{P}$ for which the premise holds, our goal is to 
...
Let us now create a 3rd slogan about Lilac ``Hilbert Cubes with Finite Footprint are infinite random generators''. More specifically
a resource $\mathcal{P}$ (which is an FFHC plus a probability measure) can simultaneously support an arbitrary number of random variables, 

allow the construction
of

% \[
% (0.1, 0.1, 0.1) \mapsto 0.1
% (0.1, 0.1, 0.2) \mapsto 0.2
% (0.1, 0.1, 0.3) \mapsto 0.3

% \]

The infinite part is what is important denoting the smallest ``trivial'' measurable space consisting of just 

decomposing a \texttt{MeasurableSpace} on the Hilbert Cube as the product of measurable space on  

The above isomorphism on the underlying type can be lifted to an isomorphism on measurable spaces by stipulating isomorphisms between associated
$\sigma$-algebras.
