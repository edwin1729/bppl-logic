...
all the data required to define it (a \lean{MeasurableSpace}). So by analogy with classical separation logic, a Probability
...
aka random variables) from the \( \Omega \xrightarrow{m} ty.den \) for some \( ty : Ty \). Which means that the distribution (the measure) over \( ty.den\) is a 
...
\subsubsection{Separation is probabilistic independence}
...
\subsubsection{Measurable Spaces and Random Variables on the Hilbert Cube}
...
$\Sigma_{I^\mathbb{N}}$.
Motivated by the aim of reasoning about random variables, $ \Omega \xrightarrow{m} A$ (where $A$ is some \texttt{ty.den}) we define:
$RV A \triangleq I^{\mathcal{N}} \xrightarrow{m} A \text{such that the map is finite footprint}$. At the moment the finite footprint is an additional restriction that does not
concern us. Measurable Spaces are implicit in the standard mathematical notation, so note that RV is defined w.r.t $\Sigma_{I^\mathbb{N}}$ (\lean{MeasurableSpace.pi}) on $I^{\mathbb{N}}$.



The following names are chosen to represent, deterministic/random values/distributions:
...
\subsubsection{Ownership is measurability}
...
& E \sim \mu \mid \mathrm{own}\,E \mid E \overset{\mathrm{as}}{\equiv} E \mid \mathbb{E}[E] = e \mid \mathrm{wp}(M,X\!:\!A.\,Q)
\end{aligned}
\]
...
This simplifies our presentation here, omitting deterministic contexts and variables, and the two quantifiers $\forall$ and $\exists$ above.
...
measurability \( E \text{ is } \mathcal{F}\text{-measurable} \)? First let us clarify that we already know that $E$ is a measurable function $RV E \triangle I^{\mathbb{N}} \xrightarrow{m} A$ so is
...
% general defintion of Finite footprint. Perhaps category theory/ synthetic measure theory can help
