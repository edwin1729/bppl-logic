...
across to the reader, without getting weighed down by the technicalities of the Lean code. \texttt{wp\_bind} is chosen since it prominently features a different style of reasoning
that is a significant aspect in many of the \texttt{wp} proof rules' soundness. \texttt{wp\_meas} has many measure theoretic proof obligations falling more into the mathematician's
plate, while in \texttt{wp\_bind} a major portion of the proof employs equational reasoning (on probability kernels), which is more familiar to the computer scientists. We employ
...
in any weakest precondition based separation logic. However, it is deeply intertwined with the semantics of \texttt{wp}, adding the extra book keeping of
...
\Todo{Draw the kernel diagram and explain just what is needed of the proof for that to make sense}
