...
\label{chap:proofs}
\emph{Having overcome the fundamental change in the definitions, consolidated inconsistencies and instantiated Iris, we present here the 2nd aspect
...
were invented for validating these proof rules, and aren't of general mathematical interest to be supported by Mathlib.
...
makes up the measurable sets (or events) in the space. Each line represents an infinite tuple of reals in the unit interval, connecting dots which are single reals,
...
other words Finite footprint hilbert cube (FFHC) can't own every random variable and is restricted in some way. In particular, we are always able to easily construct
...
the state of a program, we are always able to modify this state to some $\mathcal{P'}$ which supports one more random variable $Y$ of our choice. Each of these $Y$s is
just a straightforward projection of some dimension n, i.e. measure on each dimension is the source of randomness. And we can repeat this process arbitrarily, because
...
Firstly we see that neither measure associated with the resources $\mathcal{P}$ or $\mathcal{P'}$ get used directly. Instead, we use $\mu$ and $\mu'$ respectively which are arbitrary larger measures
...
lists all the \texttt{wp} rules of Lilac. The soundness of these rules has been proven in Lean. However, we cannot go through all of them in this report.
...
``kernel API'', Mathlib's preferred style~\citep{remy} for expressing these kinds of equations. The ``kernel API'' has a vast library of lemmas, and one can imagine
how using a point free style facilitates prerequisites for applying a lemma to be more easily inferred (fewer parameters, fewer things to infer), by the workhorse of type-class inference~\citep{spitters}.
...
$M$ has type isomorphic to that of a Kernel\footnote{\texttt{toMK} is a manual coercion to a kernel, which is required since denotations of Appl programs are not in general kernels, only those with Appl type \texttt{Ty.g ty}}.
...
In our presentation of transformations on the Hilbert cube, we are being quite verbose in stating the isomorphisms (measurable equivalence), by ``splitting'' and ``unsplitting'' the Hilbert cube. In the
...
Continuing with the proof, we feed $\mu$ into the \textcolor{ForestGreen}{outer \texttt{wp}}, to obtain a $\mu X$ and $X$ such that this equation holds:
