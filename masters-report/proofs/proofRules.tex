...
to achieve the goal (given above in black) in Fig \ref{fig:bind-proof}. Whenever a proof step is unexplained, it is simply a topological
transformation.
...
be generalised or adapted for the new proof obligations.
...
We had hoped to present the specific gaps in Mathlib's infrastructure in full detail. However, doing so introduces a great deal of notation and many
reasoning steps, only to arrive at a rather technical point in the grand scheme of the formalisation. So instead we give a sketch of the proof, and focus on the
single measure-theoretic result at its core. That result exposes an incompatibility between Mathlib (and mainstream mathematics) and our requirements, one which
has plagued most proofs across the development to some extent.
...
We want to prove a \texttt{wp} statement. Take some $\mathcal{P}$ for which the premise holds; our goal is to validate
$\mathcal{P} \vDash \mathrm{wp} (\lambda\ \_ \mapsto \mu) ( \lambda X\!:\!A.\mapsto Q)$. Unfolding the semantics of \texttt{wp} from Sec \ref{sec:wp}, this
requires us to construct a new resource $\mathcal{P'}$ that owns a random variable $X$ whose pushforward distribution is $\mu$, and for which
$\mathcal{P'} \vDash Q[X]$.

The premise is exactly the tool that discharges the $Q[X]$ obligation, once we recall the semantics of the magic wand. $X \sim \mu \wand Q$ holds for a
resource $\mathcal{R}$ if, for every $\mathcal{R}'$ with $\mathcal{R}' \vDash X \sim \mu$ such that the combination $\mathcal{R}' \star \mathcal{R}$ exists, we
have $\mathcal{R}' \star \mathcal{R} \vDash Q$.

The construction is precisely the one developed in Sec \ref{sec:ff}: given the target distribution $\mu$, we build the resource $\mathcal{P}_n$ which owns the
projection $X := (\omega : \mathbb{N} \to I) \mapsto \omega\, n$ at a fresh dimension $n$ (the finite footprint of $\mathcal{P}$), so that $X_\#\mu_n = \mu$.
Since the premise holds for $\mathcal{P}$, instantiating it with this $X$ and taking $\mathcal{R}' := \mathcal{P}_n$ yields $\mathcal{P}_n \star \mathcal{P} \vDash Q$.
Setting $\mathcal{P'} := \mathcal{P}_n \star \mathcal{P}$ then satisfies both obligations at once: $\mathcal{P'}$ owns $X$ with the correct distribution
(condition (2) of Sec \ref{sec:ff}), $\mathcal{P}$ supplies condition (1), and $\mathcal{P'} \vDash Q[X]$.

The whole proof therefore rests on a single claim that Sec \ref{sec:ff} glossed over: that the independent combination $\mathcal{P}_n \star \mathcal{P}$ exists
and behaves as expected, both at the level of the underlying measurable space and at the level of the measure. We make this precise now.
...
There was a slight omission in the above presentation, regarding the presence of $\mathcal{P}_{\mathrm{frame}}$. The full semantics of \texttt{wp} also
quantifies over a frame, so the combination whose existence we must establish is in fact $\mathcal{P}_{\mathrm{frame}} \star \mathcal{P}_n \star \mathcal{P}$.
The frame is carried along unchanged and plays no role in the reasoning, so we omit it here.

We of course need this result to extend from the measurable spaces to the measures carried on them. This is the content of the file \repo{ProofRules/MeasureProduct}, where we
prove the lemmas that establish exactly this. However ideally we could generalise this result much further to just be about an isomorphism between
...
An unavoidable difficulty across much of this Lean development was incorrect type-class inference, in particular the inference of the $\mathrm{MeasurableSpace}\, \alpha$ attached to a type $\alpha$.
To see the problem, recall the tension we noted in the standard mathematical notation for measurable spaces, for example $I^\mathbb{N} \marrow A$. Both the measurable spaces, on $I^\mathbb{N}$ and on $A$, are left implicit.
Mathlib fills these in with typeclasses, and the definition of a measurable function is given like so:
...
$[\ldots]$ marks an argument that should be filled by \emph{typeclass search}, which is different from the usual \emph{type inference}. A typeclass search succeeds when it
can find or build a suitable instance, in this case some $\mathrm{MeasurableSpace}\, \alpha$. The search does not check whether the term it builds then type checks. So if
there are several instances of $\mathrm{MeasurableSpace}\, I^\mathbb{N}$ in scope, the search may pick the wrong one, and the resulting proof term will not have the type we
expect. We can fix this by passing the typeclass argument explicitly, but this only works one level deep: if our lemma calls an inner lemma, that inner lemma runs its own
typeclass search for $\mathrm{MeasurableSpace}\, \alpha$, and if it picks the wrong instance there is nothing we can do about it. The proof just fails. The only workaround
is to be very careful never to leave the wrong $\mathrm{MeasurableSpace}$ instance in the ambient proof context, and this is easy to get wrong.

This difficulty shares a root with the reusability point made above, and in principle the same fix. We are pushed into the workaround only because our lemmas are stated in terms of the
specific finite-footprint constructions, which leaves the measurable spaces to be recovered by typeclass search. The general lemmas suggested above, about how
measures match up across a measurable equivalence of spaces built from products and sums, would take their measurable spaces as plain arguments instead of leaving them to the
search, and so would make the results reusable and remove the typeclass-inference ambiguity at the same time. We did not take this route, however: generalising the lemmas would have been
going out of our way, and at this stage the priority was simply to get a working proof as soon as possible. We therefore leave it as future work.

\subsection{Reflection on workflow and use of AI}
...
at the level of automating the proofs of time-consuming but trivial helper lemmas of a few lines each, or higher. Specifically by ``helper lemma'' we actually refer
...
I should be clear that the deep parts of these proofs are my own, worked out in the usual way. Where Aristotle genuinely helped came later. I had a proof essentially
done, but it would not go through because of the typeclass-inference problem described in the previous section. I handed that to Aristotle, and it found the hack of
pulling the entire surrounding context out into a single lemma with tens of parameters, which made the proof go through. The lemma cannot be reused and it hides the
real shape of the proof, so this is only a temporary fix; the clean fix is the generalisation discussed above, which we have left for future work.

The proof of \texttt{wp\_meas} is where Aristotle was inadvertently used the
most, so we include this reflection here.
As mentioned before, one of these pure measure-theoretic results was intuitive/believable. The informal proofs swept it under the phrase ``by
definition''. Yet this particular lemma \texttt{wp\_meas\_measure\_product\_core} in \repo{ProofRules/MeasureProduct}, was a significant effort.
This was handed to aristotle as one of the insignificant ``have'' statemements to fill, without realising
how much work it actually was. 
Aristotle did make partial progress, but on a terrible idea: of reproving the entire
measurable-equivalence framework from plain isomorphisms. That generated code was abandoned.

Most of the work for this additional unexpected measure-theoretic proof obligation was developed manually, before getting stuck on the typeclass-inference problem described in the previous section.
Handing the almost complete (but frustratingly broken due to typeclass inference) Lean code to Aristotle, which ended up closing the goal! It found the hack of
pulling the entire surrounding context out into a single lemma with tens of parameters, which made the proof go through. The lemma cannot be reused and it hides the
real shape of the proof, so this is only a temporary fix; the clean fix is the generalisation discussed above, which we have left for future work.

Stepping back, AI generated code and proofs verified by a foundational theorem prover, used to be an unatainable gold-standard for reliable code-generation from inherently
untrustworthy systems. But within the span of the year it is essentially within reach now. Reflecting on my experience with it so far,
